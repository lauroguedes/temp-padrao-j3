...
class plgContentBt_socialshare extends BtPlugin {
    ...
    public function getFacebookCommentBox($link_article,$title) {
        ...
        $extra .= '</div>'; 
		if(trim($this->params->get('mail_to',''))){
			$script = '';
			...
			$script .="xmlhttp.open('GET','".JURI::base()."plugins/content/bt_socialshare/sendmail.php?title=".urlencode($title)."&link='+encodeURIComponent(response.href),true);\n";
			$script .="xmlhttp.send();";
			$script .="};";
